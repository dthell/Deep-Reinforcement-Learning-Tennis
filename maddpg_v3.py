...
# this version (v3) uses a common actor and a common critic network for all agents, the one from maddpg_agent[0]

from ddpg import DDPGAgent
import torch
import numpy as np
from utilities import soft_update, transpose_to_tensor, transpose_list, transpose_to_nested_list, hard_update
...
class MADDPG_v3:
    ...
    def get_actors(self):
        """get actors of all the agents in the MADDPG_v3 object"""
        actors = [ddpg_agent.actor for ddpg_agent in self.maddpg_agent]
        return actors

    def get_target_actors(self):
        """get target_actors of all the agents in the MADDPG_v3 object"""
        target_actors = [ddpg_agent.target_actor for ddpg_agent in self.maddpg_agent]
        return target_actors

    def act(self, obs_all_agents, noise=[0.0, 0.0]):
        """get actions from all agents in the MADDPG_v3 object"""
        actions = [agent.act(obs, float(noise_i)) for agent, obs, noise_i in zip(self.maddpg_agent, obs_all_agents, noise)]
        return actions

    def target_act(self, obs_all_agents, noise=0.0):
        """get target network actions from all the agents in the MADDPG_v3 object """
        target_actions = []
    ...
    # the learning uses only one centralized critic network, the one from self.maddpg_agent[0]
    ...
    def copy_actor(self, id_of_source_actor):
        """hard update worst performing actor networks to match best performing actor network"""
        id_of_target_actor = (id_of_source_actor + 1) % 2

        hard_update(self.maddpg_agent[id_of_target_actor].target_actor, self.maddpg_agent[id_of_source_actor].target_actor)
        hard_update(self.maddpg_agent[id_of_target_actor].actor, self.maddpg_agent[id_of_source_actor].actor)
